class SeedDemoData < ActiveRecord::Migration[8.0]
  def up
    # Only run in production or when explicitly requested
    return unless Rails.env.production? || ENV['FORCE_DEMO_SEED'] == 'true'
    
    puts "Seeding demo data in #{Rails.env}..."
  ...
  def down
    ...
    CnfEmail.destroy_all
    RndClaim.destroy_all
    Company.destroy_all
  end
end