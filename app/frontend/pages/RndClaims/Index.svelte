<script>
  ...
  let { user, rnd_claims, search, sort_by, sort_order, can_create_claims, can_manage_claims, stats, companies } = $props();
  ...
  function deleteProject(projectId, projectTitle) {
    if (confirm(`Are you sure you want to delete "${projectTitle}"? This action cannot be undone.`)) {
      router.delete(`/rnd_claims/${projectId}`, {
        onSuccess: () => {
...
<Layout {user}>
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center mb-4">
        <div>
          <h1 class="text-3xl font-bold text-base-content mb-2">R&D Claims</h1>
          <p class="text-base-content/70">Manage research and development claims for tax credit applications</p>
        </div>
        {#if can_create_claims}
          <Button variant="primary" onclick={() => navigateTo('image.png/rnd_claims/new')}>
    ...
    <div class="bg-base-100 rounded-lg shadow border border-base-300 overflow-hidden">
      {#if rnd_claims && rnd_claims.length > 0}
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="text-xs opacity-50">
                      {claim.duration_days} days
                    </div>
                  </td>
                  <td>
                    <div class="font-medium">{formatCurrency(claim.total_expenditure)}</div>
                    {#if claim.can_be_claimed}
                  ...
                  <td>
                    <div class="flex items-center space-x-2">
                      <button 
                        onclick={() => navigateTo(`/rnd_claims/${claim.id}`)}
                      ...
                      {#if can_manage_claims}
                        <button 
                          onclick={() => navigateTo(`/rnd_claims/${claim.id}/edit`)}
                          class="btn btn-sm btn-ghost"
                          title="Edit Project"
                        >
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button 
                          onclick={() => deleteProject(claim.id, claim.title)}
      ...
      {:else}
        <div class="text-center py-12">
          <div class="text-base-content/50 mb-4">
            ...
            <h3 class="text-lg font-medium text-base-content mb-2">No R&D claims found</h3>
            <p class="text-base-content/70 mb-6">
              {searchQuery ? 'Try adjusting your search terms' : 'No R&D claims have been created yet'}
            </p>
            {#if can_create_claims}
              <Button variant="primary" onclick={() => navigateTo('/rnd_claims/new')}>
                Create First R&D Claim
              </Button>
            {/if}
          </div>
        </div>
