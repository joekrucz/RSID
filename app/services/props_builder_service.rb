  ...
  def self.rnd_claim_props(claim)
    {
      ...
      cnf_deadline: (cd = claim.cnf_deadline; cd = Date.parse(cd) if cd.is_a?(String); cd&.strftime("%Y-%m-%d")),
      cnf_submission_date: (csd = claim.cnf_submission_date; csd = Date.parse(csd) if csd.is_a?(String); csd&.strftime("%Y-%m-%d")),
